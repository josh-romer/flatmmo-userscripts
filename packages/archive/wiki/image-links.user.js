...
(() => {
	...
	async function getExcludedPageLinks() {
		...
		try {
			...
			if (navboxTable) {
				const fileSpans = navboxTable.querySelectorAll(
					'span[typeof="mw:File"]',
				);
		...
		link.addEventListener("mouseenter", async (e) => {
			...
			if (!linkCache[fileUrl]) {
				try {
					...
					const parser = new DOMParser();
					const doc = parser.parseFromString(html, "text/html");
					const linksContainer = doc.querySelector(
					...
					if (linksContainer) {
						...
						const navboxInUsageList = allLinks.some(
							(a) =>
						...
						if (filteredLinks.length > 0) {
							linkCache[fileUrl] =
								`<ul>${filteredLinks.map(linkToUL).join("")}</ul>`;
						} else {
							linkCache[fileUrl] =
								'<div style="color: #666;">No other pages use this file</div>';
						}
					} else {
						linkCache[fileUrl] =
							'<div style="color: #666;">No links found</div>';
					}
				} catch (error) {
					console.log(error);
					linkCache[fileUrl] =
						'<div style="color: #d33;">Error loading links</div>';
				}
			}

			// Display cached links
			modal.innerHTML =
				'<div style="font-weight: bold; margin-bottom: 8px; color: #202122;">Pages using this file:</div>' +
				linkCache[fileUrl];
		});
