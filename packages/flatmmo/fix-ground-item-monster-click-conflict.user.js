...
function mouse_click_handler_patched(e) {
	...
	const clicked_tile = get_postition_from_pixel(relativeX, relativeY);

	//dev tool
	if (tile_marker_mode) {
		for (let i = 0; i < tiles_marked.length; i++) {
			const tile = tiles_marked[i];
	...
	for (let i = 0; i < ground_items.length; i++) {
		const ground_item = ground_items[i];
		if (is_mouse_on_ground_item(clicked_tile.x, clicked_tile.y, ground_item)) {
			...
			if (
				Object.values(npcs).some(
					(n) =>
						!n.is_hidden &&
						is_mouse_on_npc(mouse_over_now.x, mouse_over_now.y, n),
				) &&
				e.which !== 3
			) {
				break;
	...
	const npcs_clicked_uuid = [];
	for (const uuid in npcs) {
	...
	for (let i = 0; i < map_objects.length; i++) {
		const map_object = map_objects[i];
	...
	for (let i = 0; i < teleport_tiles.length; i++) {
		const teleport_tile = teleport_tiles[i];

		if (
			teleport_tile.x == clicked_tile.x &&
			teleport_tile.y == clicked_tile.y
		) {
	...
	send_unrepeatable_bytes(
		"CLICKED_TILE=" + clicked_tile.x + "~" + clicked_tile.y,
	);
}
